...
import { dateTimeLabel } from '../data/matchData'
import {
...
const emptySlot = {
  ...
  match_date: '', match_time: '', stadium: '', competition: '',
...
function MatchForm({ slot, label, data, onSave, uploading }) {
  ...
  return (
    <Box>
      ...
      {/* Match details */}
      <Grid templateColumns={{ base: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }} gap={3} mt={4}>
        <FormControl>
          <FormLabel {...labelStyle}>Fecha</FormLabel>
          <Input {...inputStyle} value={form.match_date} onChange={set('match_date')} placeholder="19-AGO" />
        </FormControl>
        <FormControl>
          <FormLabel {...labelStyle}>Hora</FormLabel>
          <Input {...inputStyle} value={form.match_time} onChange={set('match_time')} placeholder="16:00" />
        </FormControl>
        <FormControl>
...
function MatchPreviewCard({ slot, data, label, onEdit, onDelete }) {
  ...
  const dateLabel = dateTimeLabel(data)
